...
struct tracker {
  struct tracker_entry {
    ...
    std::array<int64_t, 5> strides = {0, 0, 0, 0, 0}; // History of last 5 strides (adjustable)
  ...
  struct lookahead_entry {
    uint64_t address = 0;
    std::array<int64_t, 5> strides = {0, 0, 0, 0, 0}; // Track multiple strides
  ...
  constexpr static int STRIDE_HISTORY_SIZE = 5; // Updated number of past strides to consider
