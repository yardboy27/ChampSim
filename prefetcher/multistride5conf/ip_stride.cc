...
struct tracker {
  struct tracker_entry {
    ...
    std::map<int64_t, int> confidence_table; // Confidence counter per stride
  ...
  struct lookahead_entry {
    uint64_t address = 0;
    std::array<int64_t, 5> strides = {0, 0, 0, 0, 0};
    int degree = 0;
  };
...
public:
  void initiate_lookahead(uint64_t ip, uint64_t cl_addr)
  {
    ...
    if (found.has_value()) {
      ...
      auto &confidence_table = found->confidence_table;
      confidence_table[new_stride]++;
      
      // Check if the stride has high confidence
      if (confidence_table[new_stride] >= CONFIDENCE_THRESHOLD) {
